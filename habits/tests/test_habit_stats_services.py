...
def test_with_habit_stats__annotates_total_checkins_checked_in_today_last_7_days_count():
...
def test_total_checkins__uses_annotation_when_present():
    today = timezone.localdate()
    habit = Habit.objects.create(name="Read")
...
def test_total_checkins__falls_back_to_db_when_annotation_missing():
    today = timezone.localdate()
    habit = Habit.objects.create(name="Meditate")
    _bulk_create_checkins(habit, [today])
    obj = Habit.objects.get(pk=habit.pk)

    assert habit_stats.total_checkins(obj) == 1

def test_checked_in_today__uses_annotation_when_present():
    today = timezone.localdate()
...
def test_checked_in_today__falls_back_to_db_when_annotation_missing():
    today = timezone.localdate()
    habit = Habit.objects.create(name="Stretch")
    _bulk_create_checkins(habit, [today])
...
def test_last_7_days_count__uses_annotation_when_present():
    today = timezone.localdate()
    habit = Habit.objects.create(name="Jornal")
    _bulk_create_checkins(
        habit,
        [
            today,
...
def test_last_7_days_count__falls_back_to_db_when_annotation_missing():
    today = timezone.localdate()
    habit = Habit.objects.create(name="DrinkWater")
...
def test_current_streak__prefetched__returns_zero_when_no_checkin_today_and_hits_no_db():
    today = timezone.localdate()
    habit = Habit.objects.create(name="NoToday")
...
def test_current_streak__prefetched__counts_consecutive_days_and_hits_no_db():
    today = timezone.localdate()
    habit = Habit.objects.create(name="Streaky")
    _bulk_create_checkins(
        habit,
        [
            today,
...
def test_best_streak__prefetched__returns_max_run_and_hits_no_db():
    today = timezone.localdate()
    habit = Habit.objects.create(name="BestStreak")
...
def test_best_streak__non_prefetched__returns_max_run():
    today = timezone.localdate()
    habit = Habit.objects.create(name="BestStreakDB")
